...
def update_image(epd, config):
    ...
    if config['display']['orientation'] == 0 or config['display']['orientation'] == 180:
        if photo_image.height / photo_image.width >= 1.5: # If photo height is larger than the screen
            ...
            photo_image = photo_image.resize((176, height), Image.LANCZOS)
            # Crop
            if height > 264:
                upper, lower = (height+264)/2, (height-264)/2
            else:
                upper, lower = 264, 0
            photo_image = photo_image.crop((0, lower, 176, upper))
        else: # If photo width is larger than the screen
            # Resize
            width = round(photo_image.width * 264 / photo_image.height)
            photo_image = photo_image.resize((width, 264), Image.LANCZOS)
            # Crop
            if width > 176:
                upper, lower = (width+176)/2, (width-176)/2
            else:
                upper, lower = 176, 0
